...
After predicting $\tilde{x}__{i+1}$, we evaluate the model again to get $b(\tilde{x}_{i+1}, t_{i+1})$.
Then we correct the trajectory using a trapezoidal- or Adams–Moulton-like term:

```math
\begin{align*}
x_{i+1} &= G_{i+1,i} x_i + &h_i \sum_{k=1}^{p} \alpha_k G_{i+1,i+1-k} b(x_{i+1-k}, t_{i+1-k}) \\
